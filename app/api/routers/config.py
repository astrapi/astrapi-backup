...
def _list_response(request, storage_key):
    from api.routers.run import get_running
    return templates.TemplateResponse(
        "partials/list_wrapper.html",
        {
...
def enable_disable_router(storage_key: str):
    from fastapi import APIRouter, Request
    r = APIRouter(tags=[storage_key])

    @r.post("/enable-all")
    def enable_all(request: Request):
        for item_id, item in load_config(storage_key).items():
            if not item.get("enabled", False):
                item["enabled"] = True
                save_item(storage_key, item_id, item)
        return _list_response(request, storage_key)
