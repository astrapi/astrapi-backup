...
def _run(item_id: str, entry: dict, lock: threading.Lock) -> None:
    try:
        ...
        from astrapi_backup.modules.borg.ui.archives import (
            _get_target_info,
            _list_archives,
            _load_archive_entries,
            _repo_info,
        )
        ...
        if not to_load:
            ...
            log(
                "INFO",
                f"[cache] {len(to_load)} neue Archive laden ({len(archives) - len(to_load)} bereits gecacht)",
            )
        ...
        log(
            "INFO",
            f"[cache] Cache für Job {item_id} fertig ({len(archives)} Archive, {len(to_load)} neu geladen)",
        )
    except Exception as e:
