...
#   1. modules/borg/templates/  → "borg/..." (klassisch, falls vorhanden)
#   2. modules/borg/            → "borg/..." (neu: dialogs/ direkt unter Modul-Ordner)
# Beide Roots werden als ChoiceLoader kombiniert, damit Templates aus beiden Orten
# unter demselben Prefix erreichbar sind.
...
for _search_root in (_APP_ROOT / "modules", _CORE_MODULES):
    ...
    for _mod_dir in sorted(_search_root.iterdir()):
        ...
        if _tpl_dir.is_dir():
            _roots.append(FileSystemLoader(str(_tpl_dir)))
        # Modul-Ordner selbst als Root (für dialogs/ direkt unter dem Modul)
        _roots.append(FileSystemLoader(str(_mod_dir)))
        _prefix_loaders.append(PrefixLoader({_mod_dir.name: ChoiceLoader(_roots)}))
...
_env = Environment(
    loader=ChoiceLoader(_prefix_loaders + _base_loaders), autoescape=True, cache_size=0
)
