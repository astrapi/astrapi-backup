...
from backupctl.api.storage import load_config as _load_config, patch_item as _patch_item
from backupctl.modules.borg.utils import borg_bin_for as _borg_bin_for, borg_env as _borg_env
...
def _get_host_info(entry: dict, host_type: str = "source") -> tuple[str, str, int]:
    ...
    if remote_id_key in entry and entry[remote_id_key]:
        from backupctl.modules.remotes.engine import get_remote_ssh
        try:
...
def run_single(job_id, entry=None):
    ...
    with log_context("borg", job_id):
        ...
        from backupctl.modules.borg import cache as _cache
